...
<div>
	...
	<div class="detail-preview" :style='{"width":"1200px","margin":"10px auto","position":"relative"}'>
		<div class="attr" :style='{"padding":"20px","position":"relative","flexWrap":"wrap","background":"#f5f5f5","display":"flex"}'>
			...
			<div class="info" :style='{"padding":"10px","boxShadow":" 0px 4px 10px 0px rgba(0,0,0,0.302)","margin":"0 10px 0 10px","borderRadius":"20px","alignItems":"center","flexDirection":"column","flex":"1","background":"#ffff","display":"flex","width":"800px"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#00b9ea","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="isAuth('huochexinxi','预订')" @click="onAcross('chepiaoyuding','','','')" type="warning">预订</el-button>
				</div>
			</div>
		...
		<el-tabs class="detail" :style='{"border":"1px solid #DCDFE6","boxShadow":"0 1px 6px 0 rgba(0, 0, 0, .1)","background":"#FFF"}' v-model="activeName" type="border-card">
			<el-tab-pane label="列车路线" name="first">
				<div v-html="detail.liecheluxian"></div>
			</el-tab-pane>
  ...
  export default {
    ...
    methods: {
        init() {
          ...
          this.$http.get(this.tablename + '/detail/'  + this.detail.id, {}).then(res => {
            if (res.data.code == 0) {
              ...
              this.detailBanner = this.detail.liechetupian ? this.detail.liechetupian.split(",") : [];
              this.$forceUpdate();
            }
          });
      ...
      onAcross(acrossTable,crossOptAudit,statusColumnName,tips,statusColumnValue){
        localStorage.setItem('crossTable',`huochexinxi`);
      ...
      getStoreupStatus(){
        if(localStorage.getItem("Token")) {
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'huochexinxi', userid: localStorage.getItem('userid')}}).then(res => {
      ...
      getDiscussList(page) {
        this.$http.get('discusshuochexinxi/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discusshuochexinxi/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
                this.getDiscussList(1);
                this.$message({
